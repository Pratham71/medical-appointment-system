...
[x] [TOFIX] Emergency alert context fields — POST /emergency/alert now requires reason and location, accepts optional contact number/message, stores context in MySQL, and shows context in student/admin UI (GitHub issue #46)
[x] [TOFIX] Emergency alert acknowledgement and resolution flow — admin/staff PATCH endpoints acknowledge/resolve alerts with optional resolution notes, alert status is exposed to admins and students, and UI shows Unread/Acknowledged/Resolved states (GitHub issue #43)
...
[x] [TOFIX] [BACKEND] Add admin dashboard and role-management APIs — POST /auth/signup defaults to student/patient, professor role uses the student workflow, and admin-only routes cover dashboard metrics, user role assignment, appointments, student/professor directory, doctor/staff directories, and emergency alert review (GitHub issue #11)
