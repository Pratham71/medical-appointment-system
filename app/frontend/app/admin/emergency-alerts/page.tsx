...
function AlertCard({
  a, actionBusy, resolutionNote, onAcknowledge, onResolve, onNoteChange,
}: {
  a: AdminEmergencyAlertSummary;
  actionBusy: number | null;
  resolutionNote: Record<number, string>;
  onAcknowledge: (id: number) => void;
  onResolve: (id: number) => void;
  onNoteChange: (id: number, val: string) => void;
}) {
  return (
    <div className="bg-white rounded-card border-l-4 border-red-400 border-t border-r border-b border-brand-border shadow-card p-4">
          ...
          {a.status !== "resolved" && (
            <>
              ...
              <input
                ...
                onChange={(e) => onNoteChange(a.alert_id, e.target.value)}
...
export default function AdminEmergencyAlertsPage() {
  ...
  const handleAcknowledge = async (alertId: number) => {
    setActionBusy(alertId); setError("");
    try { replaceAlert(await acknowledgeEmergencyAlert(alertId)); }
    catch (e: unknown) { setError(e instanceof Error ? e.message : "Failed to acknowledge"); }
  ...
  const handleResolve = async (alertId: number) => {
    ...
    try {
      ...
      setResolutionNote((cur) => ({ ...cur, [alertId]: "" }));
                    ...
                    {group.rows.map((a, i) => (
                      <motion.div key={a.alert_id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.18, delay: Math.min(i * 0.04, 0.25) }}>
                        <AlertCard
                          ...
                          onNoteChange={(id, val) => setResolutionNote((cur) => ({ ...cur, [id]: val }))}
                        />
                      </motion.div>
                    ))}
                  </div>
