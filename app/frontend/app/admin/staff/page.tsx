          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-brand-border">
                {loading && (
                  <tr>
                    <td colSpan={5} className="px-5 py-8 text-center text-brand-muted text-sm animate-pulse">
                      Loading…
                    </td>
                  </tr>
                )}
                {!loading && staff.map((s) => (
                  <tr key={s.staff_id} className="hover:bg-brand-raised transition-colors">
                    ...
                    <td className="px-5 py-3">
                      {s.is_doctor ? (
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-teal-50 text-teal-700 ring-1 ring-teal-200">
                          Doctor
                        </span>
                      ) : (
                ...
                {!loading && staff.length === 0 && !error && (
                  <tr>
                    <td colSpan={5} className="px-5 py-8 text-center text-brand-muted text-sm">
                      No staff found
                    </td>
                  </tr>
                )}
